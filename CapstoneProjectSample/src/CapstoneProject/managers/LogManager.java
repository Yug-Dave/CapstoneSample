...
public class LogManager {
    ...
    public static synchronized void viewLogsByFilter(String filterType, String filterValue) {
        ...
        switch (filterType.toLowerCase()) {
            case "object" -> filteredLogs.addAll(
                logs.stream().filter(log -> log.getObjectName().equalsIgnoreCase(filterValue)).toList());
            case "battery" -> filteredLogs.addAll(
                logs.stream().filter(log -> log.getBatteryName().equalsIgnoreCase(filterValue)).toList());
            default -> {
    ...
    public static synchronized void viewLogsByDate(Date date) {
        ...
        List<LogEntry> filteredLogs = logs.stream()
            .filter(log -> log.getTimestamp().startsWith(dateString))
            .toList();
    ...
    private static void displayLogs(List<LogEntry> logs) {
        ...
        for (int i = 0; i < logs.size(); i++) {
            ...
            System.out.printf("| %-5d | %-15s | %-15s | %-20s | %-20s |\n", i, log.getObjectName(), log.getBatteryName(), log.getAction(), log.getTimestamp());
        }
    }
    ...
    public static synchronized void exportLogs(String filePath) {
        try (PrintWriter writer = new PrintWriter(new FileWriter(filePath))) {
            ...
            for (LogEntry log : logs) {
                writer.printf("%s,%s,%s,%s\n", log.getObjectName(), log.getBatteryName(), log.getAction(), log.getTimestamp());
            }
